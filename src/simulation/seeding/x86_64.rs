...
#[inline]
#[target_feature(enable = "avx2")]
fn widen_diffs(diffs: __m128i) -> __m256i {
...
/// Build both 16-entry signed-byte subset-sum tables used by Buchholz lookup.
///
/// The first table covers opponent-set bits for teams 0-7, split across the two
/// 128-bit lanes; the second does the same for teams 8-15.
#[inline]
#[target_feature(enable = "avx2")]
fn build_tables(diffs: __m128i) -> [__m256i; 2] {
    ...
    [
        build_table(diffs, consts::TABLES[0]),
        build_table(diffs, consts::TABLES[1]),
    ]
}

/// Build two adjacent 16-entry signed-byte subset-sum tables in parallel.
///
...
#[inline]
#[target_feature(enable = "avx2")]
fn build_table(diffs: __m256i, masks: [__m256i; 4]) -> __m256i {
    _mm256_add_epi8(
        _mm256_add_epi8(
...
fn pack(remaining: Set, diff: __m256i, buchholz: __m256i) -> PackedSeeding {
    ...
    let vec = _mm256_or_si256(
        _mm256_or_si256(
            _mm256_slli_epi16::<10>(diff),
            _mm256_slli_epi16::<5>(buchholz),
        ),
        consts::INDICES,
    );
    ...
    let vec = unsafe {
        ...
        let low = _mm_loadl_epi64(ByteMasks::low_ptr(remaining).cast());
        let high = _mm_loadl_epi64(ByteMasks::high_ptr(remaining).cast());
        let mask = _mm256_cvtepi8_epi16(_mm_unpacklo_epi64(low, high));

        // Blend active packed keys with `u16::MAX` sentinels for eliminated teams.
        _mm256_or_si256(
            _mm256_and_si256(mask, vec),
    ...
    unsafe {
        _mm256_store_si256(packed.as_aligned_mut_ptr().cast(), vec);
    }
